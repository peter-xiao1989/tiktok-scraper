...
async function readCols(token, sheet, a, b) {
  let out = [], s = 2;
  while (s < 4000) { const r = await api('GET', `/open-apis/sheets/v2/spreadsheets/${SS}/values/${sheet}!${a}${s}:${b}${s + 499}?valueRenderOption=FormattedValue`, token); const rows = r.data?.valueRange?.values || []; if (!rows.length) break; out = out.concat(rows); if (rows.length < 500) break; s += 500; }
  return out;
}
