...
async function maintainAllDerived(token, label = '数据') {
  // 读 uqJEhq 一次,过滤近60天,所有衍生表共享(累计列已移到 Bitable 公式字段)
  ...
  console.log(`  uqJEhq: 共${allAd.length}行, 近60天${recentAd.length}行`);

  const steps = [
    ['项目维度经营表',    () => ensureProjectSummary(token, recentAd)],
    ['产品经营日报表',    () => ensureReportFormulas(token, recentAd)],
    ['日经营数据汇总',    () => ensureDailySummary(token, recentAd)],
    ['投放日表-产品维度', () => ensureAdProductSummary(token, recentAd)],
